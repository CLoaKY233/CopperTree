...
def get_or_seed_case(borrower_id: str) -> CaseFile:
    ...
    if doc:
        ...
        if case.stage not in (Stage.ASSESSMENT, Stage.RESOLUTION, Stage.FINAL_NOTICE):
            print(f"[!] Case is at stage '{case.stage}' — resetting to ASSESSMENT for a fresh run.")
...
def save_case(case: CaseFile) -> None:
    ...
    db.case_files.update_one(
        ...
        {"$set": {**case.model_dump(), "updated_at": datetime.now(timezone.utc).isoformat()}},
    )
...
def run_pipeline(borrower_id: str) -> None:
    ...
    print(f"\n{'='*65}")
    print(f"  CopperTree Live Pipeline")
    ...
    if case.stage == Stage.ASSESSMENT:
        print_banner(1, "ASSESSMENT", "Text chat — the agent verifies your identity and assesses the debt.")
        ...
        messages, case, flags = agent.run_conversation(case_file=case, io=io, budget=budget)
    ...
    if case.stage == Stage.RESOLUTION:
        print_banner(2, "RESOLUTION", "Voice call — speak via your mic, hear the agent through your speakers.")
        ...
        system_prompt = system_prompt.replace("{{debt_amount}}", f"{_cur}{case.debt.amount:,.2f}")
        system_prompt = system_prompt.replace("{{creditor}}", case.debt.creditor)
        system_prompt = system_prompt.replace("{{account_ending}}", case.partial_account or "")
        ...
        call_result = voice_client.run_session(system_prompt=full_system, borrower_id=borrower_id)
    ...
    if case.stage == Stage.FINAL_NOTICE:
        print_banner(3, "FINAL NOTICE", "Text chat — last opportunity before legal referral.")
        print("  Type your replies below. Type 'quit' to end early.\n")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Run the full CopperTree pipeline with you as the borrower.")
    parser.add_argument("--borrower-id", default="live_borrower_001", help="Borrower ID to load/seed")
    parser.add_argument("--reset", action="store_true", help="Reset borrower case to ASSESSMENT stage")
    ...
    if args.reset:
        ...
        db.case_files.update_one(
            ...
            {"$set": {"stage": "assessment", "compliance": {}, "dispute_validation_required": False}},
        )
        db.transcripts.delete_many({"borrower_id": args.borrower_id})
        print(f"Reset: {args.borrower_id} → assessment")
